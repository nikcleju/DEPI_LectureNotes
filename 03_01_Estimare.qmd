...
(de ex. amplitudinea e necunoscută).
Semnalul fiind afectat de zgomot, la receptor ajunge semnalul recepționat
următor:

$$r(t) = s_\Theta(t) + zgomot$$

În teorie estimării, vrem să găsim valoarea parametrului $\Theta$,
cunoscând semnalul recepționat și parametrii statistici ai zgomotului.

Întrucât nu cunoaștem valorile exacte ale zgomotului,
nu putem găsi valiarea adevărată a parametrului necunoscut,
tot ce putem face este să îl estimăm cât mai bine.
Valoarea găsită în urma estimării va fi notată
$\hat{\Theta}$, și se numește **estimatul* lui $\Theta$

Va există întotdeauna o eroare de estimare $\epsilon$ între
valoarea adevărată a lui $\Theta$ și
estimatul de la recepție:
...
problemele de decizie și cele de estimare.
Practic, prin "decizie" înțelegem o problemă
de estimare **restrânsă** doar la un set discret, redus, de opțiuni;
se alege doar una dintre cele câteva ipoteze.
La rândul său, o problemă de estimare
poate fi văzută ca o problemă de detectție, dar
cu un număr **infinit de opțiuni** posibile.
În fundal, însă, metodele statistice folosite sunt foarte similare.
