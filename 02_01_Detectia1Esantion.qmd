...
O problemă de decizie înseamnă a decide între două alternative, în prezența zgomotului.
...
Avem la dispoziție semnale **cu zgomot** aditiv (se adună la semnalul original)

O schema bloc a unui sistem de comunicații este prezentată mai jos.
...
Fracția $\frac{w(r|H_1)}{w(r|H_0)}$ poartă numele de **raportul de plauzibilitate**.
...
ceea ce înseamnă că ignoră probabilitatea celor două ipoteze, $P(H_0)$ și $P(H_1)$.
Un alt criteriu de decizie, intitulat criteriul probabiității minime de eroare,
ia în calcul și pe acestea.
...
Se observă că criteriul MPE este o generalizare a criteriului ML,
plauzibilitățile $w(r|H_0)$ și $w(r|H_1)$ fiind înmulțite,
suplimentar, cu probabilitățile $P(H_0)$ și $P(H_1)$.
...
Vrem să minimizăm **riscul**, care reprezintă **costul mediu**, calculat astfel:
...
rezultatul unei comparației, adică dacă $A < B$, atunci $\log(A) < \log(B)$.
...
Termenul $|r-s_0(t_0|$ este chiar **distanța** goemetrică,
pe axa numerelor reale, dintre $r$ și $s_0(t_0)$,
iar $|r-s_1(t_0)|$ distanța de la $r$ la $s_1(t_0)$.
...
Pentru criteriul ML, $K=1$, și relația se reduce chiar la alegerea
...
Menționăm că luarea unei decizii pe baza **cel mai apropiat vecin** ("*nearest neighbor*"),
este un principiu foarte general, foarte des întâlnit in literatura de specialitate, sub diverse alte forme.
...
2. Dacă zgomotul are distribuție diferită în ipoteza $H_0$ față de ipoteza $H_1$?
...
De exemplu, fie o problemă de decize binară cu $s_0(t) = 0$, $s_1(t) = 10$,
și zgomot $\mathcal{N}(\mu=0, \sigma^2 = 4)$.
Fie o altă problemă de decizie cu $s_0(t) = 10$, $s_1(t) = 16$, and noise $\mathcal{U}[-8, 8]$

Care dintre cele două probleme e mai ușoară?
Va trebui să comparăm probabilitățile "bune" ($P_{cd}$, $P_{cr}$)
și cele "rele" ($P_{fa}$, $P_m$).
...
a curbei ROC. Aceasta duce la o valoare care caracterizează întregul sistem,
...
ca $\frac{\text{puterea semnalului util}}{\text{puterea zgomotului}}$

Performanțele oricărei probleme de decizie depind în mod fundamental
de raportul semnal-zgomot al sistemului.

A îmbunătăți performanțele unei probleme de decizie binară
înseamnă, practic, a crește $P_d$ menținând $P_{af}$ la fel.
...
1. Scăderea zgomotului, adicăscade puterea zgomotului.

Ambele soluții înseamnă, de fapt, creșterea **raportul Semnal-Zgomot**:
