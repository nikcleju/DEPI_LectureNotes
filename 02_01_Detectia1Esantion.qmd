...
## Detecția semnalelor folosind 1 eșantion
...
- Pentru 0 logic se transmite semnalul
  $$s_0(t) = 0$$
- Pentru 1 logic se transmite semnalul
  $$s_1(t) = 5$$

Pe canalul de transmisie se suprapune zgomot.
La recepție se ia un singur eșantion $r$, și valoarea sa este $3.5$.

Ce decizie luăm? Am recepționat un 0 sau un 1?
...
Decizia se ia alegând ipoteza cu **cea mai mare plauzibilitate**
pentru valoarea $r$. Matematic, aceasta se scrie:

$$\frac{L(H_1 | r)}{L(H_0 | r)} = \frac{w(r|H_1)}{w(r|H_0)} \grtlessH 1$$

Expresia Se alege valoarea maximă dintre $w(r(t_0)|H_0)$ și $w(r(t_0) | H_1)$
...
$$\frac{w(r|H_1)}{w(r|H_0)} = \frac{e^{-\frac{(r-s_1(t_0))^2}{2\sigma^2}}}{e^{-\frac{(r-s_0(r_0))^2}{2\sigma^2}}} \grtlessH 1$$
...
Așadar, putem spune că pentru zgomot gaussian criteriul ML
înseamnă alegerea **distanței minime** $r(t_0)$ și $s_1(t_0)$ sau $s_0(t_0)$.

Cu alte cuvinte, decizia este câștigată de semnalul a cărui valoare,
$s_0(t_0)$ sau $s_1(t_0)$, este cea mai apropiată de $r = r(t_0)$.
...
Recapitulând, etapele pentru a lua o decizie pe baza criteriului ML sunt:
...
În cazul în care zgomotul este de tip gaussian, identic pentru toate ipotezele,
atunci lucrurile se simplifică:
 ...
 3. Se alege **cea mai apropiată** valoare de $r(t_0)$
...
De exemplu, pentru zgomot gaussian, există un singur pragul $T = \frac{s_0(t_0) + s_1(t_0)}{2}$
...
Problema de decizie analizată până acum se poate generaliza.
